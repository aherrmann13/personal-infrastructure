...
output "vault_server_private_ips" {
  value       = [for server in digitalocean_droplet.vault-server : server.ipv4_address_private]
  description = "list of the consul server private ip addresses"
}

output "vault_server_public_ips" {
  value       = [for server in digitalocean_droplet.vault-server : server.ipv4_address]
